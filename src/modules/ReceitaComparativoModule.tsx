...
export default function ReceitaComparativoModule() {
	...
	const ov = CR.origem
		...
		.sort((x, y) => y.p - x.p);
	const cagr = ((462.4 / 360) ** (1 / 4) - 1) * 100;
	return (
		<>
			<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-4">
				<KpiCmp
					label="Arrecadada (total)"
					a={CR.arr25}
					b={CR.arr26}
					accent={t.primary}
				/>
					...
					<Title
						right={
							<span className="text-xs" style={{ color: t.mutedFg }}>
								Acum. Jan–Jun por exercício
							</span>
						}
					>
						Evolução da Arrecadação (2022–2026)
					</Title>
					<div style={{ height: 240 }}>
						<ResponsiveContainer width="100%" height="100%">
							<AreaChart
								data={CR.evol.map(([ano, v]) => ({ ano, Arrecadada: v }))}
					...
					<Title
						right={
							<div className="flex gap-4 text-xs">
								<LegendDot color={prev}>2025</LegendDot>
								<LegendDot color={cur}>2026</LegendDot>
							</div>
						}
					>
						Arrecadação por mês — 2025 × 2026
					</Title>
					<GroupedBars
						data={CR.meses.map(([mes, a, b]) => ({ mes, a2025: a, a2026: b }))}
						height={240}
					/>
				</Card>
				<Card className="p-5">
					<Title>Variação YoY por Origem (%)</Title>
					<Diverging data={ov} height={240} ylabel={110} />
				</Card>
			</div>
			<Card className="p-5 mb-4">
				<Title>Arrecadação por Categoria › Origem › Espécie</Title>
				<TreeCmp
					nodes={CR.arvore}
					level0="Categoria"
					totalLabel="Receita Bruta Arrecadada"
					tot25={CR.arr25}
					tot26={CR.arr26}
				/>
			</Card>
