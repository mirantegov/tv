...
import { brl, dP, dR, fmt, fmtInt, pct, vari } from "./format";
...
export function Delta({
	...
	return (
		<span
			...
			style={{ color: up ? t.primary : t.warn, fontWeight: 600 }}
		>
			{up ? "▲" : "▼"} {money ? dR(d) : dP(p)}
		</span>
	);
	...
	return (
		<Card className="p-4">
			...
			<div
				...
				style={{ color: accent || t.foreground }}
			>
				{value}
			</div>
			{progress != null && (
				<div
					...
					style={{ height: 5, background: t.secondary }}
				>
					<div
						style={{
							height: "100%",
			...
			{sub && (
				<div className="text-xs mt-2 tabular-nums" style={{ color: t.mutedFg }}>
					{sub}
				</div>
			)}
		</Card>
	...
	return (
		<Card className="p-4">
			...
			<div className="flex items-center gap-2 mt-2 text-xs">
				<Delta {...v} />
				<span className="tabular-nums" style={{ color: t.mutedFg }}>
					{dR(v.d)} mi
				</span>
			</div>
			<div className="text-xs mt-1 tabular-nums" style={{ color: t.mutedFg }}>
				2025: {brl(a)}
			</div>
		</Card>
	);
				...
				>
					...
					<Bar dataKey="valor" name="Valor" radius={[0, 4, 4, 0]}>
						{data.map((_, i) => (
							<Cell
								key={i}
								fill={highlightFirst && i === 0 ? t.primary : cats[3]}
							/>
						))}
					</Bar>
				</BarChart>
			...
			<ResponsiveContainer width="100%" height="100%">
				...
				>
					...
					<YAxis
						type="category"
						dataKey="nome"
						tick={{ fontSize: 9.5, fill: t.foreground }}
						axisLine={false}
						tickLine={false}
						width={ylabel}
					/>
					<Tooltip
						...
						content={({ active, payload }: any) =>
							active && payload?.length ? (
								...
								>
									<b>{payload[0].payload.nome}</b>: {dP(payload[0].payload.p)} (
									{dR(payload[0].payload.d)} mi)
								</div>
							) : null
						}
					/>
					...
					<Bar dataKey="p" radius={[2, 2, 2, 2]}>
						{data.map((o, i) => (
							<Cell key={i} fill={o.up ? t.primary : t.warn} />
						))}
					</Bar>
				</BarChart>
			</ResponsiveContainer>
...
export function LegendDot({
	...
	const { t } = useTheme();
	return (
		<span className="flex items-center gap-1.5" style={{ color: t.mutedFg }}>
			<span
...
function flatten(
	...
	for (const n of nodes) {
		...
		if (n.children && exp[n.id]) flatten(n.children, exp, depth + 1, acc);
	}
	return acc;
}
export function TreeCmp({
	nodes,
	level0,
	totalLabel,
	tot25,
	...
	return (
		<div className="overflow-x-auto">
			...
			>
				...
				<tbody>
					{rows.map(({ node, depth }) => {
						const v = vari(node.v25, node.v26);
						const hasK = !!node.children;
						const open = exp[node.id];
						return (
							<tr
								key={node.id}
								onClick={() =>
									hasK && setExp((e) => ({ ...e, [node.id]: !e[node.id] }))
								}
								style={{
									borderTop: `1px solid ${t.border}`,
									background: depth === 0 ? t.muted : "transparent",
					...
					<tr style={{ borderTop: `2px solid ${t.border}` }}>
						...
						<td
							className="text-right tabular-nums"
							style={{ padding: "9px 10px", fontWeight: 700, color: t.primary }}
						>
							{dR(tot26 - tot25)}
						</td>
						<td className="text-right" style={{ padding: "9px 10px" }}>
							<Delta {...vari(tot25, tot26)} />
						</td>
					</tr>
				</tbody>
			</table>
			...
			>
				<thead>
					<tr style={{ color: t.mutedFg }}>
						...
						<th
							className="text-left"
							style={{ padding: "8px 10px", fontWeight: 600, width: 140 }}
						>
							% Realiz.
						</th>
						<th
				...
				<tbody>
					{rows.map(({ node, depth }) => {
						const hasK = !!node.children;
						const open = exp[node.id];
						const p = node.prev !== 0 ? (node.real / node.prev) * 100 : 0;
						const saldo = node.prev - node.real;
						return (
							<tr
								key={node.id}
								onClick={() =>
