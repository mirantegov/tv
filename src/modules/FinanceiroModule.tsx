	...
	return (
		<>
				...
				>
					...
					<div className="text-sm" style={{ color: t.foreground }}>
						<b style={{ color: t.danger }}>
							{insuf.length} fonte com insuficiência financeira
						</b>{" "}
						— {insuf.map((x) => `${x.nome} (${dR(x.suf)} mi)`).join(", ")}.
						Vedado usar recursos de outra fonte para cobrir (LRF art. 50, I).
					</div>
				</div>
			)}
				...
				<Card className="p-5 lg:col-span-2">
					<Title
						right={
							<div className="flex gap-4 text-xs">
								<LegendDot color={cats[1]}>Ingressos</LegendDot>
								<LegendDot color={t.warn}>Desembolsos</LegendDot>
								<LegendDot color={cur}>Saldo acum.</LegendDot>
							</div>
						}
					>
					...
					<div style={{ height: 250 }}>
						<ResponsiveContainer width="100%" height="100%">
							<ComposedChart
								data={F.fluxo.map(([mes, ing, des], i) => ({
									mes,
									Ingressos: ing,
									Desembolsos: des,
									Saldo: F.evol[i][1],
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
						...
						<ResponsiveContainer width="100%" height="100%">
							...
							>
								...
								<Bar dataKey="valor" name="Disp. líquida" radius={[0, 4, 4, 0]}>
									{F.fontes.map((f, i) => (
										<Cell key={i} fill={f[1] ? cats[3] : t.primary} />
									))}
								</Bar>
							</BarChart>
						</ResponsiveContainer>
				...
				<Card className="p-5">
					<Title>Composição por Banco</Title>
					<Donut
						data={F.bancos.map(([n, v]) => ({ nome: n, valor: v }))}
						height={210}
					/>
				</Card>
				<Card className="p-5">
					<Title>Obrigações por Natureza</Title>
					<HBar
						highlightFirst
						data={F.obrigacoes.map(([n, v]) => ({ nome: n, valor: v }))}
						height={150}
						ylabel={120}
					/>
					<div className="mt-3">
						<Title>Pagamentos por tipo</Title>
						<HBar
							data={F.pagamentos.map(([n, v]) => ({ nome: n, valor: v }))}
							height={150}
							ylabel={120}
						/>
					</div>
			...
			<Card className="p-5 mb-4">
				<Title
					right={
						<span className="text-xs" style={{ color: t.mutedFg }}>
							RGF · Anexo 5 · valores em R$ milhões
						</span>
					}
				>
									...
									<tr key={i} style={{ borderTop: `1px solid ${t.border}` }}>
										...
										<td
											...
											style={{
												padding: "9px 10px",
												fontWeight: 700,
												color: suf < 0 ? t.danger : t.ok,
											}}
										>
											{suf < 0 ? `(${fmt(Math.abs(suf))})` : fmt(suf)}
										</td>
									</tr>
								);
							})}
				...
				<div className="text-xs mt-3" style={{ color: t.mutedFg }}>
					Suficiência apurada <b>por fonte</b>: o total positivo não compensa a
					insuficiência de uma fonte vinculada/livre específica.
				</div>
			</Card>
					...
					>
						<tbody>
							{[
								...
								["(−) Dispêndios extraorçamentários", -82.0, t.warn],
							].map((r, i) => (
								<tr
									key={i}
									style={{ borderTop: i ? `1px solid ${t.border}` : "none" }}
								>
									<td style={{ padding: "8px 4px", color: t.foreground }}>
										{r[0]}
									</td>
									<td
										className="text-right tabular-nums"
										style={{ padding: "8px 4px", color: r[2] }}
									>
						...
						>
							<thead>
								<tr style={{ color: t.mutedFg }}>
									<th
										className="text-left"
										style={{ padding: "7px 6px", fontWeight: 600 }}
									>
										Natureza
									</th>
									<th
							...
							<tbody>
								{F.extra.map(([n, ret, rec], i) => (
									<tr key={i} style={{ borderTop: `1px solid ${t.border}` }}>
										<td style={{ padding: "8px 6px", color: t.foreground }}>
											{n}
										</td>
										<td
											className="text-right tabular-nums"
											style={{ padding: "8px 6px", color: t.mutedFg }}
										>
											{fmt(ret)}
										</td>
										<td
