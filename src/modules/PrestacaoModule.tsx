	...
	const Dot = ({ s, size = 13 }) => (
		<span
			style={{
				...
				background: s === "ok" ? t.ok : s === "no" ? t.danger : t.mutedFg,
				opacity: s === "na" ? 0.4 : 1,
			}}
		/>
	);
				...
				<>
					{note(
						<>
							...
							, prestadas via SIM-AM (mensal) e SIM-AP (anual). Prazos conforme
							a Instrução Normativa vigente do TCE-PR — confirme o calendário do
							exercício.
						</>,
					)}
					...
					<Section
						...
						desc="Compromissos do município junto ao TCE-PR, por entidade."
					>
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
							<Kpi
								label="Em Dia"
								value={fmtInt(C.tce.agenda.kpis.emDia)}
								accent={t.ok}
								sub="obrigações atendidas"
							/>
							<Kpi
								label="Não Atendido"
								value={fmtInt(C.tce.agenda.kpis.naoAtendido)}
								accent={t.danger}
								sub="exige regularização"
							/>
							<Kpi
								label="Não Aplicável"
								value={fmtInt(C.tce.agenda.kpis.naoAplicavel)}
								sub="fora do escopo do ente"
							/>
							<Kpi
								label="Entidades"
								value={fmtInt(C.tce.agenda.kpis.entidades)}
								sub="unidades monitoradas"
							/>
						</div>

						<div className="flex items-center gap-5 mb-3 text-xs flex-wrap">
							<span style={{ color: t.mutedFg, fontWeight: 600 }}>
								Situação:
							</span>
							<span
							...
							>
								<Dot s="ok" size={11} /> Em dia
							</span>
							<span
								className="flex items-center gap-2"
								style={{ color: t.foreground }}
							>
								<Dot s="no" size={11} /> Não atendido
							</span>
							<span
								className="flex items-center gap-2"
								style={{ color: t.foreground }}
							>
								<Dot s="na" size={11} /> Não aplicável
							</span>
						</div>
						...
						<Card className="p-5">
							...
							<div className="text-xs mb-4" style={{ color: t.mutedFg }}>
								Compromissos do município junto ao TCE/PR — {C.tce.agenda.local}
								, {C.tce.agenda.periodo}
							</div>
							<div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-3">
								{C.tce.agenda.colunas.map(([sg, desc], i) => (
									<div key={i} className="flex items-start gap-3">
										...
										<span
											className="text-xs"
											style={{ color: t.mutedFg, lineHeight: 1.5 }}
										>
											{desc}
										</span>
									</div>
					...
					<Section
						num="2"
						title="Certidão"
						desc="Certidão Liberatória do TCE-PR."
					>
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
							<Kpi
								label="Situação"
							...
							<Kpi
								label="Pendências"
								value={fmtInt(C.tce.certidao.kpis.pendencias)}
								sub="nenhuma restrição"
							/>
							<Kpi
								label="Tipo"
								value={C.tce.certidao.kpis.tipo}
								sub="emissão TCE-PR"
							/>
						</div>
						<Card className="p-5">
					...
					<Section
						num="3"
						title="Contas Anuais"
						desc="Prestação de Contas Anual (PCA), parecer prévio do TCE e julgamento pela Câmara."
					>
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
							<Kpi
								label="Exercício em Análise"
								value={C.tce.contas.kpis.exercicio}
								accent={t.primary}
								sub="PCA enviada"
							/>
							<Kpi
								label="Parecer Prévio"
								value={C.tce.contas.kpis.parecer}
								sub="aguardando o TCE"
							/>
							<Kpi
								label="Entrega da PCA"
								value={C.tce.contas.kpis.entrega}
								accent={t.ok}
								sub="dentro do prazo"
							/>
							<Kpi
								label="Tempestividade"
								value={C.tce.contas.kpis.tempestividade}
								sub="últimos 5 exercícios"
							/>
						</div>
						<Card className="p-5">
							<div className="overflow-x-auto">
			...
			{aba === "siconfi" && (
				<>
					{note(
						<>
							Obrigações com o{" "}
							<b style={{ color: t.foreground }}>SICONFI / Tesouro Nacional</b>.
							O <b style={{ color: t.foreground }}>CAUC</b> consolida as
							exigências fiscais para transferências voluntárias; a{" "}
							<b style={{ color: t.foreground }}>MSC</b> (Matriz de Saldos
							Contábeis) é a base mensal que gera RREO, RGF e DCA.
						</>,
					)}
					...
					<Section
						num="1"
						title="CAUC"
						desc="Serviço Auxiliar de Informações para Transferências Voluntárias — checklist fiscal."
					>
						<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
							<Kpi
								label="Itens Regulares"
								value={`${C.siconfi.cauc.kpis.regulares} / ${C.siconfi.cauc.kpis.total}`}
								accent={t.ok}
								sub="exigências atendidas"
							/>
							<Kpi
								label="Pendentes"
								value={fmtInt(C.siconfi.cauc.kpis.pendentes)}
								accent={t.warn}
								sub="bloqueiam convênios"
							/>
							<Kpi
							...
							<Kpi
								...
								value={C.siconfi.cauc.kpis.verificacao}
								sub="consulta ao CAUC"
							/>
						</div>
						<Card className="p-5">
							<div className="grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-1">
								{C.siconfi.cauc.itens.map(([ex, to], i) => (
									...
									>
										...
										<span
											className="text-xs"
											style={{ color: t.foreground, flex: 1 }}
										>
											{ex}
										</span>
