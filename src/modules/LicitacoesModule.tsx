	...
	return (
		<>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
					...
					<Title
						right={
							<div className="flex gap-3 text-xs">
								<LegendDot color={cats[6]}>Estimado</LegendDot>
								<LegendDot color={cats[0]}>Homologado</LegendDot>
								<LegendDot color={t.ok}>Economia</LegendDot>
							</div>
						}
					>
						Estimado × Homologado × Economia
					</Title>
					<div style={{ height: 240 }}>
						<ResponsiveContainer width="100%" height="100%">
							<BarChart
								data={LIC.mensal.map(([mes, est, hom]) => ({
									mes,
									Estimado: est,
									Homologado: hom,
									Economia: +(est - hom).toFixed(1),
								}))}
				...
				<Card className="p-5">
					<Title>Situação dos Processos</Title>
					<Donut
						numeric
						data={LIC.situacao.map(([n, v]) => ({ nome: n, valor: v }))}
						height={210}
					/>
				</Card>
			</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
				<Card className="p-5">
						...
						{LIC.funil.map(([label, val], i) => (
							<div key={i}>
								...
								<div
									...
									style={{ height: 22, background: t.secondary }}
								>
									<div
										style={{
											height: "100%",
											width: `${(val / LIC.funil[0][1]) * 100}%`,
					...
					<div className="text-xs mt-3" style={{ color: t.mutedFg }}>
						Conversão publicado → contratado:{" "}
						{pct((LIC.funil[3][1] / LIC.funil[0][1]) * 100)}.
					</div>
				</Card>
				<Card className="p-5">
					<Title>Compras por Objeto</Title>
					<HBar
						highlightFirst
						data={LIC.objeto.map(([n, v]) => ({ nome: n, valor: v }))}
						height={230}
						ylabel={112}
					/>
				</Card>
			...
			<div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
				<Card className="p-5">
					...
					>
						Destino do Valor Homologado — ME/EPP
					</Title>
					<Donut
						data={LIC.meepp.map(([n, v]) => ({ nome: n, valor: v }))}
						height={190}
					/>
					<div className="text-xs mt-2" style={{ color: t.mutedFg }}>
						<b style={{ color: t.ok }}>
							{pct((LIC.meepp[0][1] / LIC.homologado) * 100)}
						</b>{" "}
						do valor ficou com ME/EPP do próprio município (
						{fmtInt(LIC.meeppExclusivos)} certames com disputa exclusiva/cota) —
						compras públicas como fomento à economia local.
					</div>
				</Card>
				<Card className="p-5">
					<Title
					...
					{(() => {
						const tot =
							LIC.pncp.noPrazo + LIC.pncp.foraPrazo + LIC.pncp.pendentes;
						const rows = [
							["Publicados no prazo", LIC.pncp.noPrazo, t.ok],
							["Publicados fora do prazo", LIC.pncp.foraPrazo, t.warn],
							["Pendentes de publicação", LIC.pncp.pendentes, t.danger],
						];
						return (
							<div className="flex flex-col gap-3 mt-2">
								<div className="flex items-baseline gap-2">
									<span
										...
										style={{ color: t.ok }}
									>
										{pct((LIC.pncp.noPrazo / tot) * 100)}
									</span>
									<span className="text-xs" style={{ color: t.mutedFg }}>
										dos {fmtInt(tot)} processos no prazo
									</span>
								</div>
								{rows.map(([lb, v, c], i) => (
									<div key={i}>
								...
								<div className="text-xs" style={{ color: t.mutedFg }}>
									Sem publicação no PNCP o contrato não tem eficácia — pendência
									é bloqueio, não burocracia.
								</div>
							</div>
						);
					})()}
				...
				<Card className="p-5">
					...
					<div className="flex flex-col gap-3 mt-2">
						{LIC.atas.map(([ob, reg, cons, venc], i) => (
							<div key={i}>
								<div className="flex items-baseline justify-between mb-1">
									...
									>
										{fmt(cons)} / {fmt(reg)} · vence {venc}
									</span>
								</div>
								<div
									className="rounded-full overflow-hidden"
									style={{ height: 8, background: t.secondary }}
								>
									<div
										style={{
											height: "100%",
											width: `${(cons / reg) * 100}%`,
											background: cons / reg > 0.85 ? t.warn : t.primary,
											borderRadius: 999,
										}}
									/>
								</div>
						...
						<div className="text-xs" style={{ color: t.mutedFg }}>
							Ata perto de esgotar (ou de vencer) sem novo certame planejado
							vira compra emergencial — acompanhar consumo evita a dispensa por
							urgência fabricada.
						</div>
					</div>
				</Card>
			...
			<Card className="p-5 mb-4">
				...
				<div className="text-xs mt-2" style={{ color: t.mutedFg }}>
					A "economia" só é real se a pesquisa de preços (valor estimado) for
					confiável — estimativa inflada gera economia fictícia.
				</div>
			</Card>
